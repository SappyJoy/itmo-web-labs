...
function getXbyOffset(offset) {
  return (offset - (x_axis_distance_grid_lines * grid_size)) / grid_size;
}

function getYbyOffset(offset) {
  return -(offset - (y_axis_distance_grid_lines * grid_size)) / grid_size;
}
...
function draw_area(r) {
  ...
  ctx.lineTo(-r * grid_size, 0);
  ctx.fill();

  // Third quarter
  ctx.beginPath();
  ctx.moveTo(0, 0);
  ctx.arc(0, 0, r * grid_size, Math.PI, Math.PI / 2, true);
  ctx.fill();
